...
export default function ClassGroupEdit({ classGroup, terms, teachers }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        term_id: classGroup.term_id,
        name: classGroup.name,
        level: classGroup.level,
        stage: classGroup.stage,
        homeroom_teacher_user_id: classGroup.homeroom_teacher_user_id || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/class-groups/${classGroup.id}`);
    };

    return (
        <AppLayout title="Edit Kelas" showBack={true} backUrl="/admin/class-groups">
